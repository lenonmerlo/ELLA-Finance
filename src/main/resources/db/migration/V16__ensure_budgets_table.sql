-- Ensure budgets table exists (drift recovery)
--
-- Why: some local/dev databases already have Flyway versions 13-15 applied to *different* scripts
-- (version reuse / history drift). In that scenario, Flyway will not execute V13__create_budgets_table.sql,
-- and Hibernate may fail with: Schema-validation: missing table [budgets].
--
-- This migration is safe/idempotent and forces the physical table to exist.
...
-- Optional non-unique index used by the JPA entity (@Index(name="idx_budget_owner"))
