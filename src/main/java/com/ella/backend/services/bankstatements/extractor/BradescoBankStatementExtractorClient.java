...
@Component
public class BradescoBankStatementExtractorClient {

    private static final Logger log = LoggerFactory.getLogger(BradescoBankStatementExtractorClient.class);
    ...
    @Autowired
    public BradescoBankStatementExtractorClient(
            @Value("${ella.extractor.base-url:http://localhost:8000}") String baseUrl
    ) {
        this(baseUrl, createDefaultRestTemplate());
    }

    BradescoBankStatementExtractorClient(String baseUrl, RestTemplate restTemplate) {
        this.restTemplate = restTemplate;
        String url = (baseUrl != null && !baseUrl.isBlank()) ? baseUrl.trim() : "http://localhost:8000";
        if (url.endsWith("/")) url = url.substring(0, url.length() - 1);
        this.baseUrl = url;
        log.info("[BradescoBankStatementExtractorClient] Configured baseUrl={}", this.baseUrl);
    }

    public BradescoBankStatementResponse parseBradescoBankStatement(byte[] pdfBytes) {
        if (pdfBytes == null || pdfBytes.length == 0) {
            throw new IllegalArgumentException("pdfBytes is empty");
        }

        String url = baseUrl + "/parse/bradesco-bank-statement";
        ...
        try {
            ResponseEntity<BradescoBankStatementResponse> response = restTemplate.postForEntity(url, request, BradescoBankStatementResponse.class);
            BradescoBankStatementResponse parsed = response.getBody();
            if (parsed == null) {
            ...
            String msg = "ella-extractor error (Bradesco statement) status=" + e.getStatusCode()
    ...
    public record BradescoBankStatementResponse(
            String bank,
