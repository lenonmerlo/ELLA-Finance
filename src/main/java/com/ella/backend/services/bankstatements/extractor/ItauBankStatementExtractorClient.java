...
@Component
public class ItauBankStatementExtractorClient {

    private static final Logger log = LoggerFactory.getLogger(ItauBankStatementExtractorClient.class);
    ...
    @Autowired
    public ItauBankStatementExtractorClient(
            @Value("${ella.extractor.base-url:http://localhost:8000}") String baseUrl
    ) {
        this(baseUrl, createDefaultRestTemplate());
    }

    ItauBankStatementExtractorClient(String baseUrl, RestTemplate restTemplate) {
        this.restTemplate = restTemplate;
        String url = (baseUrl != null && !baseUrl.isBlank()) ? baseUrl.trim() : "http://localhost:8000";
        if (url.endsWith("/")) url = url.substring(0, url.length() - 1);
        this.baseUrl = url;
        log.info("[ItauBankStatementExtractorClient] Configured baseUrl={}", this.baseUrl);
    }

    public ItauBankStatementResponse parseItauBankStatement(byte[] pdfBytes) {
        if (pdfBytes == null || pdfBytes.length == 0) {
            throw new IllegalArgumentException("pdfBytes is empty");
        }

        String url = baseUrl + "/parse/itau-bank-statement";
        ...
        try {
            ResponseEntity<ItauBankStatementResponse> response = restTemplate.postForEntity(url, request, ItauBankStatementResponse.class);
            ItauBankStatementResponse parsed = response.getBody();
            if (parsed == null) {
            ...
            String msg = "ella-extractor error (Itau statement) status=" + e.getStatusCode()
    ...
    public record ItauBankStatementResponse(
            String bank,
