...
@Component
public class BancoDoBrasilBankStatementExtractorClient {

    private static final Logger log = LoggerFactory.getLogger(BancoDoBrasilBankStatementExtractorClient.class);
    ...
    @Autowired
    public BancoDoBrasilBankStatementExtractorClient(
            @Value("${ella.extractor.base-url:http://localhost:8000}") String baseUrl
    ) {
        this(baseUrl, createDefaultRestTemplate());
    }

    BancoDoBrasilBankStatementExtractorClient(String baseUrl, RestTemplate restTemplate) {
        this.restTemplate = restTemplate;
        String url = (baseUrl != null && !baseUrl.isBlank()) ? baseUrl.trim() : "http://localhost:8000";
        if (url.endsWith("/")) url = url.substring(0, url.length() - 1);
        this.baseUrl = url;
        log.info("[BancoDoBrasilBankStatementExtractorClient] Configured baseUrl={}", this.baseUrl);
    }

    public BancoDoBrasilBankStatementResponse parseBancoDoBrasilBankStatement(byte[] pdfBytes) {
        if (pdfBytes == null || pdfBytes.length == 0) {
            throw new IllegalArgumentException("pdfBytes is empty");
        }

        String url = baseUrl + "/parse/banco-do-brasil-bank-statement";
        ...
        try {
            ResponseEntity<BancoDoBrasilBankStatementResponse> response = restTemplate.postForEntity(url, request, BancoDoBrasilBankStatementResponse.class);
            BancoDoBrasilBankStatementResponse parsed = response.getBody();
            if (parsed == null) {
            ...
            String msg = "ella-extractor error (Banco do Brasil statement) status=" + e.getStatusCode()
    ...
    public record BancoDoBrasilBankStatementResponse(
            String bank,
