...
@Component
public class C6BankStatementExtractorClient {

    private static final Logger log = LoggerFactory.getLogger(C6BankStatementExtractorClient.class);
    ...
    @Autowired
    public C6BankStatementExtractorClient(
            @Value("${ella.extractor.base-url:http://localhost:8000}") String baseUrl
    ) {
        this(baseUrl, createDefaultRestTemplate());
    }

    C6BankStatementExtractorClient(String baseUrl, RestTemplate restTemplate) {
        this.restTemplate = restTemplate;
        String url = (baseUrl != null && !baseUrl.isBlank()) ? baseUrl.trim() : "http://localhost:8000";
        if (url.endsWith("/")) url = url.substring(0, url.length() - 1);
        this.baseUrl = url;
        log.info("[C6BankStatementExtractorClient] Configured baseUrl={}", this.baseUrl);
    }

    public C6BankStatementResponse parseC6BankStatement(byte[] pdfBytes) {
        if (pdfBytes == null || pdfBytes.length == 0) {
            throw new IllegalArgumentException("pdfBytes is empty");
        }

        String url = baseUrl + "/parse/c6-bank-statement";
        ...
        try {
            ResponseEntity<C6BankStatementResponse> response = restTemplate.postForEntity(url, request, C6BankStatementResponse.class);
            C6BankStatementResponse parsed = response.getBody();
            if (parsed == null) {
            ...
            String msg = "ella-extractor error (C6 statement) status=" + e.getStatusCode()
    ...
    public record C6BankStatementResponse(
            String bank,
